...
## 프로젝트 제약 사항 및 보완점
### 제약 사항
- RAG 구현에 사용할 Docs의 활용처/자료를 구하지 못하여 제외
- AWS의 리소스를 사용하여 실제 시연 시 AWS Credential이 필요함.
    - 팀 내 Testbed 사용으로 인해, `CloudAutomation팀/박창헌 매니저`에게 연락 주시거나, server/main.py 에 있는 `/test/inject_message` API를 사용하셔야 합니다.
 
## 마무리
기존에 생각만하고 있던 기획이라 구체화와 진행을 하다 보니, 최종 과제에서 원하는 청사진과 많이 달라진 것 같으나 양해 부탁드립니다.
이번 과제를 진행하며 개발 경험이 많지 않고, 업무와 함께 진행하다보니 시간이 부족해 기능 설계나 어플리케이션 아키텍처에 대해 많이 미흡한 것 같습니다.
덕분에 이번에 LLM 뿐만 어플리케이션 구조 등 여러모로 많은 지식과 교훈을 얻게 된 것 같습니다.
좋은 코스 기획/진행 해주셔서 감사하고, 교육 과정 운영 하시면서 관리하시느라 고생 많으셨습니다.
